...
class TestSummarizeHourHappyPath:
    ...
    async def test_60_summaries_produce_hour_summary(
        self, hour_summarizer, mock_llm
    ):
        ...
        mock_llm.generate.side_effect = [
            _make_ten_min_response(i) for i in range(6)
        ] + [_make_hour_response()]
...
class TestPartialHour:
    ...
    async def test_30_summaries_produce_summary(
        self, hour_summarizer, mock_llm
    ):
        """30 summaries -> 3 x 10-min chunks + 1 hour call."""
        summaries = [f"Summary {i}." for i in range(30)]
        mock_llm.generate.side_effect = [
            _make_ten_min_response(i) for i in range(3)
        ] + [_make_hour_response()]
    ...
    async def test_20_summaries_produce_summary(
        self, hour_summarizer, mock_llm
    ):
        """20 summaries -> 2 x 10-min chunks + 1 hour call."""
        summaries = [f"Summary {i}." for i in range(20)]
        mock_llm.generate.side_effect = [
            _make_ten_min_response(i) for i in range(2)
        ] + [_make_hour_response()]
...
class TestSmallInput:
    ...
    async def test_5_summaries_single_llm_call(
        self, hour_summarizer, mock_llm
    ):
    ...
    async def test_1_summary_single_llm_call(
        self, hour_summarizer, mock_llm
    ):
    ...
    async def test_10_summaries_single_llm_call(
        self, hour_summarizer, mock_llm
    ):
...
class TestErrorHandling:
    ...
    async def test_ten_min_failure_raises_summarization_error(
        self, hour_summarizer, mock_llm
    ):
    ...
    async def test_ten_min_invalid_json_raises_error(
        self, hour_summarizer, mock_llm
    ):
...
class TestParallelExecution:
    ...
    async def test_prompts_contain_minute_labels(
        self, hour_summarizer, mock_llm
    ):
...
class TestTokenCounting:
    ...
    async def test_token_count_zero_for_empty(
        self, hour_summarizer, mock_llm
    ):
        result = await hour_summarizer.summarize_hour(
            recording_id=1, hour_index=0, minute_summaries=[]
