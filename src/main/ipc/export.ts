...
export function registerExportHandlers(databaseService: DatabaseService): void {
  ...
  ipcMain.handle(ExportChannels.OBSIDIAN, async (_event, recordingId: number, options: ExportOptions) => {
  ...
  ipcMain.handle(ExportChannels.BATCH, async (_event, recordingIds: number[], options: ExportOptions) => {
    const paths = await exportService.exportBatch(recordingIds, options)
    return { paths }
  })

  ipcMain.handle(ExportChannels.PREVIEW, async (_event, recordingId: number, templateName: string) => {
    const recording = databaseService.getRecordingWithTranscript(recordingId)
    if (!recording) {
      throw new Error('Recording not found')
    }
