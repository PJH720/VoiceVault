...
export function registerDiarizationHandlers(
  ...
  ipcMain.handle(
    ...
    async (_event, audioPath: string, recordingId: number): Promise<{ success: boolean; segments: SpeakerSegment[] }> => {
  ...
  ipcMain.handle(
    ...
    (_event, recordingId: number, transcriptSegments: TranscriptSegment[], speakerSegments?: SpeakerSegment[]) => {
  ...
  ipcMain.handle(DiarizationChannels.MERGE_SPEAKERS, (_event, sourceId: number, targetId: number) => {
