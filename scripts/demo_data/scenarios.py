...
Segments:
    1. Cafe Chat with Sarah   (09:00-09:45,  45 min) — conversation template
    2. Advanced AI Lecture     (10:30-12:00,  90 min) — lecture template, RAG core
    3. Lunch Chat with Friend2 (12:00-13:00,  60 min) — conversation template
    4. Solo Study at Library   (13:00-18:00, 300 min) — memo template, RAG auxiliary
"""
...
# Covers: project status, React refactoring, CI/CD, team roles, deadlines
# ---------------------------------------------------------------------------

# Key transcripts for notable minutes — non-key minutes are interpolated
...
# Corresponding per-minute summaries for the cafe segment key minutes
...
# Covers: LangChain, LCEL, ReAct agents, RAG architecture, Memory, LangSmith
# This segment provides the richest content for RAG similarity search demos.
# ---------------------------------------------------------------------------
...
# Covers: academic workload, exam prep, coding club, travel plans
# ---------------------------------------------------------------------------
...
# Covers: LangGraph (StateGraph, checkpoints, multi-agent), lecture review,
#         code practice and debugging. Longest segment with hour summaries.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Hour summaries for the 5-hour study session (segment 4 only).
# These aggregate 60 per-minute summaries into one summary per hour,
# demonstrating the hierarchical summarization pipeline.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Public scenarios list — consumed by seed_demo_data.py.
# Each entry is a complete scenario dict with metadata, key content,
# classification result, and optional hour summaries.
# ---------------------------------------------------------------------------
...
def generate_all_transcripts(scenario: dict) -> list[dict]:
    """Generate transcript entries for every minute, interpolating from key transcripts.

    Key minutes use their exact text. Non-key minutes reuse the closest
    preceding key minute's text with a slightly reduced confidence score
    to simulate realistic variation in STT output quality.
    ...
    # Build O(1) lookup: minute_index -> transcript dict
    ...
    for m in range(total):
        if m in key_map:
            # Use exact key transcript for this minute
            result.append(key_map[m])
        else:
            # Interpolate: find the closest preceding key minute
            ...
            # Slightly lower confidence for interpolated (non-key) minutes
            result.append({
                "minute_index": m,
                "text": base["text"],
...
def generate_all_summaries(scenario: dict) -> list[dict]:
    """Generate summary entries for every minute, interpolating from key summaries.

    Key minutes use their exact summary. Non-key minutes reuse the closest
    preceding key minute's summary text, keywords, and speakers, simulating
    the real pipeline where each minute gets its own LLM-generated summary.
    ...
    for m in range(total):
        if m in key_map:
            # Use exact key summary for this minute
            result.append(key_map[m])
        else:
            # Interpolate: carry forward the closest preceding key summary
            closest = key_indices[0]
            for ki in key_indices:
                if ki <= m:
                    closest = ki
